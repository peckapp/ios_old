    ...
    if (appDel.enterFromForum) {
     ...
     // self.readmoreTitleButton.hidden=YES;
       // self.tableview.tableFooterView = self.footerView;
        ...
        listofevents =   [[ASIFormDataRequest alloc]initWithURL:[NSURL URLWithString:displayCampusForumComments]] ;
...
-(void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    if (request==listofevents)
    {
        ...
        DLog(@"status=%@",[dict objectForKey:@"event_title"]);
...
- (IBAction)keyboardhidden:(id)sender{
    ...
    [self.forumComment resignFirstResponder];
      if (forumComment.text.length<=80) {
        ...
        [appDel addLoaderForViewController:self];
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
          postcomments =   [[ASIFormDataRequest alloc]initWithURL:[NSURL URLWithString:addCampusForumComments]] ;
