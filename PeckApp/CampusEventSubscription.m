...
@implementation CampusEventSubscription
...
- (IBAction)closeCampusSubscription:(id)sender{
...
-(void)viewWillAppear:(BOOL)animated
{
   ...
   // DLog(@"useridprofile=%@",appDel.userid);
    listofdepartments =   [[ASIFormDataRequest alloc]initWithURL:[NSURL URLWithString:allCampusDepartmentsdetails]] ;
...
-(void) textDidChange:(id)sender
{
    ...
    for(int i=0; i<[deparmentcount count]; i++){
        ...
        NSString *match=self.findDepartmenttextfield.text;
        match = [match lowercaseString];
        NSString *department = [[deparmentcount objectAtIndex:i] valueForKey:@"departments"];
    ...
    {
        self.findDepartmenttextfield.placeholder=@"Find a Department";
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (deparmentcount!=nil)
    {
        ...
        nameLabel.text=[[deparmentcount objectAtIndex:indexPath.row] objectForKey:@"departments"];
