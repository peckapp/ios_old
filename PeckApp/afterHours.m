...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if (appDel.enterscreen)
    {
        ...
        [appDel addLoaderForViewController:self];
         startindex=0;
         previousindex=0;
        appDel.siafterhours=0;
        appDel.piafterhours=0;
        
        self.tableview.separatorStyle=UITableViewCellSeparatorStyleNone;
        //self.tableview.hidden=YES;
        listofevents =   [[ASIFormDataRequest alloc]initWithURL:[NSURL URLWithString:afterHoursEvents]] ;
    ...
    else if (appDel.menu)
    {
        ...
        else
        {
            ...
            self.tableview.contentOffset=CGPointMake(0, appDel.afterhoursscrollercontent);
...
-(void) refreshdMoreDataToTable
{
    ...
    previousevents =   [[ASIFormDataRequest alloc]initWithURL:[NSURL URLWithString:previousAfterHours]] ;
     ...
     /*   if ([[NSString stringWithFormat:@"%d",[[dict valueForKey:@"notification_count"] intValue]]isEqual:@"0"])
        ...
        */
        ...
        if ([[dict objectForKey:@"status"] isEqual:@"s"])
        {
            ...
            appDel.siafterhours=startindex;
            ...
            [[NSUserDefaults standardUserDefaults] setObject:yourArrayAsData forKey:@"eventcountafterhours"];
            [[NSUserDefaults standardUserDefaults] synchronize];
        ...
        else if (dict==nil)
        {
            ...
            [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"eventcountafterhours"];
            [[NSUserDefaults standardUserDefaults]synchronize];
            [appDel removeLoader];
        }
      ...
      /*  if ([[NSString stringWithFormat:@"%d",[[dict valueForKey:@"notification_count"] intValue]]isEqual:@"0"])
        ...
        */
        ...
        if ([[dict objectForKey:@"status"] isEqual:@"s"])
        {
            ...
            appDel.piafterhours=previousindex;
...
-(IBAction)afterHoursClosed:(id)sender
{
    ...
    {
        appDel.afterhoursscrollercontent=self.tableview.contentOffset.y;
        frame.origin.x = 320-70;
        ...
        [menuclickbutton addTarget:self action:@selector(afterHoursClosed:) forControlEvents:UIControlEventTouchUpInside];
        [self.tableview addSubview:menuclickbutton];
        [self.tableview bringSubviewToFront:menuclickbutton];
        self.tableview.scrollEnabled=NO;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (eventcount!=nil) {
        ...
        if ([imageurl isEqual:@""])
        {
            typeImage.image = [UIImage imageNamed:@"after_hours_listing.png"];
        }
        else
        {
